...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function ensureLocalEmotePreviewUrl(emoteKey) {
    ...
    const pending = (async () => {
      const emoteData = await emoteDB.getEmote(
        emoteDataMap.get(emoteKey)?.key || emoteKey
      );
      if (!emoteData?.blob) {
  ...
  function hydrateEmoteImageElement(emoteKey, mediaElement) {
    ...
    const rehydrateAfterCachedUrlFailure = () => {
      ...
      hydrateEmoteImageElement(emoteKey, mediaElement);
    };
  ...
  async function loadSendEmoteNameSetting() {
    ...
    sendEmoteNameWithMedia = result[SEND_EMOTE_NAME_SETTING_KEY] ?? true;
    if (sendEmoteNameToggle) {
      sendEmoteNameToggle.checked = result[SEND_EMOTE_NAME_SETTING_KEY] ?? true;
    }
  }
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      ...
      (async () => {
        ...
        try {
          ...
          const cachedEmote = await emoteDB.getEmote(
            emoteDataMap.get(emoteTrigger)?.key || emoteTrigger
          );
  ...
  function createEmoteItemElement(key, emoteDbData = null) {
    ...
    const needsHydration = !emoteObjectUrlCache.has(key);
    const imageUrl = needsHydration ? PLACEHOLDER_PX : (emoteObjectUrlCache.get(key) || PLACEHOLDER_PX);
  ...
  function createTelegramImportToast(progressData = {}) {
    ...
    if (previewCount > 0) {
      parts.push(`${previewCount} animated preview${previewCount === 1 ? '' : 's'}`);
    }
    if (parts.length === 0) {
      const total = Number(progressData.importedCount || 0);
